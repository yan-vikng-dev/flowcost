...
export const buildSystemPrompt = (context: MessageContext): string => {
	...
	const basePrompt = [
		...
		"Users pair with one partner by sharing that person's contact card; the recipient then taps Accept or Decline on the invite. /unpair ends sharing. There are no pairing commands to type.",
		"</main objective>",
		"<capabilities>",
		...
		"To share with a partner, the user sends that person's contact card; pairing has no typed command.",
