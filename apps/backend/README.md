...
- **Chat handling**: Verified webhook messages route to `handleIncomingMessage`. Slash commands: `/new` resets conversation, `/help` shows hints, `/link` returns the settings URL, `/unlink` removes the link and revokes scheduling. Normal text is proxied to the `AgentServer` DO (Gemini 2.5 + PostHog tracing) which uses the Vercel AI SDK for tool calling, a system + context prompt, loop control via `stopWhen` + repeated tool-call guards, step-level message trimming, and a late-step tool cooldown with `activeTools` before returning the reply; Worker relays via WhatsApp Cloud API v19.0.
