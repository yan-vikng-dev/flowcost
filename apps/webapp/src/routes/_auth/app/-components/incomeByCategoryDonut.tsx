...
export function IncomeByCategoryDonut() {
  ...
  const { chartData, total } = React.useMemo(() => {
    const base = (data?.entries ?? [])
      ...
      .map((e) => ({ category: e.category, amount: Math.round(e.amountConverted) }))
      .sort((a, b) => b.amount - a.amount);
    const top = base.slice(0, 4);
    const rest = base.slice(4);
    const restSum = rest.reduce((acc, it) => acc + it.amount, 0);
    const display = restSum > 0 ? [...top, { category: "Everything else", amount: restSum }] : top;
  ...
  return (
    <Card className="flex flex-col">
        ...
        {isEmpty ? (
          <div className="mx-auto grid h-[300px] w-full place-items-center">
            <div className="text-center text-sm">
              <div className="mx-auto mb-3 grid h-24 w-24 place-items-center rounded-full border-2 border-dashed border-muted">
                <div className="text-muted-foreground">No data</div>
              </div>
              <div className="text-muted-foreground">No income this month</div>
              <div className="text-muted-foreground">Add income to see the breakdown</div>
            </div>
          </div>
        ) : (
          <ChartContainer
            config={chartConfig}
            className="[&_.recharts-pie-label-text]:fill-foreground mx-auto aspect-square max-h-[300px] overflow-visible"
          >
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        {/* Intentionally left available for future insights */}
      </CardFooter>
    </Card>
  );
}
...
function CenterLabel({
  ...
  }, [symbol, total, innerRadius, hPad, signPrefix, subtitle]);
      ...
      <g ref={groupRef} transform={`scale(${scale})`}>
        <text textAnchor="middle" dominantBaseline="middle">
          <tspan className="fill-foreground font-bold">
            <tspan className="text-3xl md:text-4xl font-bold">
              {signPrefix}
              {total.toLocaleString()}
            </tspan>
            <tspan className="text-xl align-baseline"> {symbol}</tspan>
          </tspan>
          <tspan x={0} dy={24} className="fill-muted-foreground">
            {subtitle}
          </tspan>
        </text>
      </g>
