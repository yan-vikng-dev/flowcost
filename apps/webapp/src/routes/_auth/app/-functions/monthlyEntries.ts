...
import { entries, type SelectEntry } from "@repo/data-ops/drizzle/schemas/entries/table";
import {
  exchange_rates,
...
export const getMonthlyEntriesForCharts = createServerFn()
  ...
  .handler(async (ctx): Promise<MonthlyEntriesResult> => {
    ...
    const neededDates = Array.from(
      new Set(found.map((e) => DateTime.fromJSDate(e.executedAt, { zone: timeZone }).toISODate())),
    ...
    const enriched: MonthlyEntryForCharts[] = found.map((e) => {
      const dateKey =
        DateTime.fromJSDate(e.executedAt, { zone: timeZone }).toISODate() || latest.date;
      ...
      const amountConverted =
        typeof src === "number" && src > 0 && typeof dst === "number" ? e.amount * (dst / src) : 0;
