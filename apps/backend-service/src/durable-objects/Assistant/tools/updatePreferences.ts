import { DrizzleDb } from "@repo/data-ops/database/setup";
import { MessageContext } from "../AiConversationServer";
import { tool } from "ai";
import { z } from "zod";
import { currencies, type Currency } from "@repo/shared-config";
...
const updatePreferencesSchema = z.object({
  timezone: z
    .string()
    .optional()
    .describe("New timezone for the user in IANA format, for example Europe/Paris"),
  displayCurrency: z
    .string()
    .regex(/^[A-Z]{3}$/)
    .optional()
...
export const makeUpdatePreferencesTool = (context: MessageContext, db: DrizzleDb) =>
  tool({
