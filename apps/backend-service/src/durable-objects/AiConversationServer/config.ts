...
export const getSystemMessage = (): ModelMessage => {
	return {
		...
		content:
			...
			"Each turn, you must choose exactly one: call a tool OR send a text response to the user.\n" +
			"CRITICAL: After executing any tool, you MUST provide a concise summary on the NEXT turn (not the same turn as the tool call). Never return empty text after tool execution. The summary should confirm the action taken and the outcome.\n" +
